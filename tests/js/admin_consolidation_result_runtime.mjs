...
// ── Bank size advisory (indicator only), both inspectors ───────────
{
    ...
    assert.ok(okJob.includes('Bank size advisory'), 'consolidation inspector must show the size advisory');
    assert.ok(okJob.includes('compaction is a human decision'), 'the advisory names the decision owner');
    ...
    assert.ok(failedAdvisoryJob.includes('Bank size advisory'), 'a failed job still shows the advisory');
    ...
    assert.equal(silent.includes('Bank size advisory'), false, 'no advisory → nothing rendered');
    ...
    assert.ok(sdInspector.includes('Bank size advisory'), 'space-detail inspector must show the size advisory');
